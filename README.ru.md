...
Прогоняет все скрипты офлайн против локального мока трёх API (без ключей и сети): валидацию, режим репетиции, оба бэкенда картинок с референсами и повторами, планировщик и QA через адаптеры Anthropic, OpenAI и Gemini, сборку вариантов и сравнение. Около 20 секунд.

## Если что-то не запускается

- **PowerShell пишет, что `npm.ps1` нельзя загрузить** — используйте `npm.cmd run demo` или вызывайте скрипты напрямую через `node ...`; либо один раз разрешите локальные скрипты: `Set-ExecutionPolicy -Scope CurrentUser RemoteSigned`.
- **PowerShell 5 не понимает `&&`** — команды по одной или через `;`.
- **После `setx` ключ «не задан»** — перезапустите терминал и Claude Code, `setx` действует только на новые процессы.
- **HTTP 429 или 5xx при генерации** — бэкенды повторяют запрос 4 раза с паузой; если не помогает, увеличьте `GEN_DELAY_MS` (по умолчанию 1500).
- **`WARNING: "<model>" is not a known ... image model`** — модель из `CARDGEN_IMAGE_MODEL` или плана предназначена другому провайдеру; задайте пару с `CARDGEN_IMAGE_PROVIDER` или флаг `--model`.
