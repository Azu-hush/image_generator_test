...
export async function runBackend(backend, argv = process.argv.slice(2)) {
  ...
  // the script you run fixes the provider; model comes from --model > plan > env > config.json > backend default
  let model;
  try { model = resolveStage("image", { cli: { provider: backend.name, model: args.model }, plan }).model; }
  catch { model = args.model || backend.defaultModel; }
  if (gen.backend && gen.backend !== backend.name && !args.model) model = backend.defaultModel;
  try {
    const known = JSON.parse(fs.readFileSync(path.join(ROOT, "models.json"), "utf8")).stages.image.models.filter((m) => m.provider === backend.name).map((m) => m.model);
    if (known.length && !known.includes(model)) console.log(`WARNING: "${model}" is not a known ${backend.name} image model (${known.join(", ")}). Check --model / CARDGEN_IMAGE_MODEL / config.json; continuing anyway.`);
  } catch {}
  ...
  for (const card of plan.cards) {
    ...
    if (refs.missing?.length) console.log(`  WARNING ${base}: no reference images for character(s) ${refs.missing.join(", ")}; add them to styleguide/characters.md`);
    ...
    const req = { stage: "image", backend: backend.name, model, card: { id: card.id, name: card.name, category: card.category, gold: card.category === 5 }, references: refFiles, settings: backend.settings(gen, card), prompt };
    ...
    for (let k = 1; k <= candidates; k++) {
      ...
      if (fs.existsSync(file) && !args.force) { entry.candidates.push(path.relative(ROOT, file).replaceAll("\\", "/")); continue; }
      if (dryRun) continue;
      ...
      try {
        ...
        entry.candidates.push(path.relative(ROOT, file).replaceAll("\\", "/"));
        calls++;
        console.log("ok");
      } catch (err) {
  ...
  // keep entries of cards that were not part of this run (e.g. --cards 7 regenerates one card)
  const merged = [...previous.filter((p) => !manifest.some((m) => m.card.id === p.card.id)), ...manifest].sort((a, b) => a.card.id - b.card.id);
  fs.writeFileSync(manifestFile, JSON.stringify(merged, null, 2));

  if (!dryRun && calls) await fitSet(outDir, { quiet: true });
