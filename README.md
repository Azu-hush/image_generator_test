...
`npm install` once: it brings in `sharp`. Every QA step and every API backend run first centre-crops candidates to the card aspect 430:480 automatically (generators return 4:5, 2:3 or 1:1; originals are kept in `candidates_src/`). `node scripts/fit.mjs --set <set> [--size card]` does the same by hand or forces exactly 430 × 480; `assemble` always outputs 430 × 480.
